...
NEWS_FEEDS = {
    ...
    "japan_news": {
        "name": "The Japan News",
        "url": "https://feeds.thejapannews.net/rss/c00.xml",
        "region": "japan",
        "type": "rss",
        "category": "international"
    },
    
    # POKEMON (Gaming/Entertainment)
    "pokemon_serebii": {
        "name": "Serebii.net Pokemon News",
        "url": "https://www.serebii.net/",  # Manual scraping
        "region": "global",
        "type": "scrape",
        "category": "gaming"
    },
    ...
    "ca_sfchronicle": {
        "name": "SF Chronicle (NorCal)",
        "url": "https://www.sfchronicle.com/rss/feed/California-News-1594.php",
        "region": "california",
        "type": "rss",
        "category": "local"
    },
    "ca_sandiego": {
        "name": "San Diego Union-Tribune (SoCal)",
        "url": "https://www.sandiegouniontribune.com/arc/outboundfeeds/rss/",
        "region": "california",
        "type": "rss",
        "category": "local"
    },
...
def fetch_rss_feed(feed_url: str, max_items: int = 10):
    """
...
def get_news_feed_items(feed_id):
    ...
    if feed_config['type'] == 'rss':
        ...
        # For sites without RSS (like Serebii), return placeholder
        return jsonify({
            'feed_id': feed_id,
            'feed_name': feed_config['name'],
            'success': True,
            'items': [{
                'title': f'{feed_config["name"]} - Visit site for latest news',
...
def get_latest_news():
    ...
    # Fetch from top feeds only to avoid slowdown
    priority_feeds = [
    ...
    return jsonify({
        'news': all_news[:50],  # Top 50 most recent
        'total': len(all_news),
        'last_updated': datetime.now().isoformat()
    })
